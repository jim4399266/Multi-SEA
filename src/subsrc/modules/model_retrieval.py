...
from .AFormer_contrast4 import AFormerContrast4
...
class RetrievalModuleWithQueueShared(BaseModule):
    def __init__(self, config):
        super().__init__()
        self.save_hyperparameters()
        image_encoder_config = config['image_encoder_config']
        ...
        self.aformer = AFormerShared(aformer_config)
        self.itm_head = nn.Linear(hidden_size, 2)
...
class RetrievalModuleWithQueueContrast1(RetrievalModuleWithQueue_1):
    '''
    不经过第二阶段
    '''
    def __init__(self, config):
    ...
    def encoding_text(self, batch):
        ...
        text_feats = self.text_proj_f(pretrained_output.pooler_output)
        text_embeds = self.text_proj_e(pretrained_output.last_hidden_state)
        return [text_feats, text_embeds, attention_mask]
    ...
    def encoding_image(self, batch):
        ...
        pretrained_output = self.visual_encoder(image)
        if hasattr(pretrained_output, 'last_hidden_state'):
            image_feats = self.vision_proj_f(pretrained_output.pooler_output)
            image_embeds = self.vision_proj_e(pretrained_output.last_hidden_state)
        else:
            image_feats = self.vision_proj_f(self.vision_pooler(pretrained_output))
            image_embeds = self.vision_proj_e(pretrained_output)
        image_atts = torch.ones(image_embeds.size()[:-1], dtype=torch.long).to(self.device)
        return [image_feats, image_embeds, image_atts]

class RetrievalModuleWithQueueContrast2(RetrievalModuleWithQueue_1):
    '''
    不使用SwiFFN
    '''
...
class RetrievalModuleWithQueueContrast4(RetrievalModuleWithQueue_1):
    '''
    ...
    def __init__(self, config):
        ...
        aformer_config.beta = config['beta']
        aformer_config.attention_probs_dropout_prob = config['drop_rate']
        self.aformer = AFormerContrast4(aformer_config)