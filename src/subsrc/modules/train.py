...
def train_irtr_with_queue_balance(pl_module, batch):
    '''
    添加负样本队列，并添加多任务平衡
    '''
    with torch.no_grad():
        pl_module.temp.clamp_(0.001, 0.5)
        pl_module.itc_coefficient.clamp_(0.1, 1)
        pl_module.itm_coefficient.clamp_(1, 10)
    alpha = pl_module.hparams.config['cur_alpha']
    ...
    # 添加多任务平衡
    loss_obj = F.mse_loss(loss_itc * pl_module.itc_coefficient, loss_itm * pl_module.itm_coefficient)
    irtr_loss = loss_itc * pl_module.itc_coefficient + loss_itm * pl_module.itm_coefficient + loss_obj

    pl_module.log(f"train/itc_loss", loss_itc)
    pl_module.log(f"train/itm_loss", loss_itm)
    pl_module.log(f"train/total_loss", irtr_loss)
    pl_module.log(f"train/itc_loss_s", loss_itc * pl_module.itc_coefficient)
    pl_module.log(f"train/itm_loss_s", loss_itm * pl_module.itm_coefficient)
    pl_module.log(f"train/total_loss_o", loss_itc + loss_itm)
    pl_module.log(f"train/temp", pl_module.temp)
    pl_module.log(f"train/itc_coefficient", pl_module.itc_coefficient)
    pl_module.log(f"train/itm_coefficient", pl_module.itm_coefficient)
